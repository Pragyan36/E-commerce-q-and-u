...
class LatestProductsViewAllScreen extends StatefulWidget {
  const LatestProductsViewAllScreen({super.key});

  @override
  State<LatestProductsViewAllScreen> createState() => _ViewAllScreenState();
}

class _ViewAllScreenState extends State<LatestProductsViewAllScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Obx(
        () {
          return landingController.latestProductsLoading.isTrue
              ? GridView.builder(
                  itemCount: 6,
              ...
              : landingController.latestProductsList.isEmpty
                  ? const NoProductWidget()
                  : GridView.builder(
                      itemCount: landingController.latestProductsList.length,
                      physics: const BouncingScrollPhysics(),
                      padding: const EdgeInsets.symmetric(
                        horizontal: 10,
                        vertical: 20,
                          ...
                          children: [
                            CategoryCard(
                              productImg: (landingController
                                  ...
                                  "",
                              price:
                                  "${(landingController.latestProductsList[index].stocks?[0].price) ?? ""}",
                              ...
                              rating: (landingController
                                  ...
                                  "",
                              specialprice:
                                  "${(landingController.latestProductsList[index].stocks?[0].specialPrice) ?? (landingController.latestProductsList[index].stocks?[0].price)}",
                              ontap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) {
                                      return ProductDetailView(
                                        ...
                                        productname: landingController
                                            ...
                                            "N/A",
                                        productprice:
                                            "${(landingController.latestProductsList[index].stocks?[0].price) ?? 0}",
                              ...
                              isFav:
                                ...
                                landingController.latestProductsList[index].id,
                              ),
                              ontapFavorite: () {
